...
namespace DX
{
	...
	public:
		...
		// When in fixed time step mode, set how often to call Update.
		...
		// call this to avoid fixed time step logic attempting a string of catch-up Update calls.
		...
		void Tick(const TUpdate& update)
		{
			...
			// Clamp excessively large time deltas (after paused in the debugger).
			...
			{
				// Fixed time step update logic.

				// If we are running very close to the target elapsed time (within 1/4 ms) we just clamp
				// the clock to exactly match our target value. This prevents tiny and irrelevant errors
	...
	private:
		...
		// For tracking the frame rate.
		uint32 m_frameCount;
		uint32 m_framesPerSecond;
		uint32 m_framesThisSecond;
		uint64 m_qpcSecondCounter;

		// For configuring fixed time step mode.
